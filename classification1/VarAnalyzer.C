...
double rhoS(TTree *t, TString var1, TString var2);
double rhoB(TTree *t, TString var1, TString var2);
...
void VarAnalyzer(double signalWgt=1.0){
  ...
  for (int i=0; i<nvar; i++){
    for (int j=i; j<nvar; j++){
      sRho->Fill(i+1,j+1,rhoS(ts,vars[i],vars[j]));
      bRho->Fill(i+1,j+1,rhoB(tb,vars[i],vars[j]));
    }
  }
...
void cutScan(TH1F *s, TH1F *b, TH1F **cutL, TH1F **cutH, TString var){
  ...
  *cutH=new TH1F("CH"+var,"C_H:"+var,s->GetNbinsX(),
  ...
  for (int i=1; i<=s->GetNbinsX(); i++){ // scan possible cut values
    int ns=0, nb=0;
    for (int j=i; j<=s->GetNbinsX(); j++){ // count events above cut
      ns+=s->GetBinContent(j);
      nb+=b->GetBinContent(j);
    }
...
double rhoS(TTree *t, TString var1, TString var2){
  ...
  t->SetBranchStatus("*",0);
  t->SetBranchStatus(var1,1);
  t->SetBranchStatus(var2,1);
  Float_t v1,v2;
  ...
  //cout << sumx << " " << sumy << " " << sumxy << " " << sumx2 << " " << sumy2 << endl;
  cout << "corrS("<<var1<<","<<var2<<")= " << corr<<endl;
  return corr;
}
...
double rhoB(TTree *t, TString var1, TString var2){
  ...
  double corr= (n*sumxy-sumx*sumy)/
    ( TMath::Sqrt((n-1)*sumx2-sumx*sumx) *
      TMath::Sqrt((n-1)*sumy2-sumy*sumy) );
  //  cout << sumx << " " << sumy << " " << sumxy << " " << sumx2 << " " << sumy2 << endl;
  cout << "corrB("<<var1<<","<<var2<<")= " << corr<<endl;
  return corr;
}
